...
func (r Retriever) Retrieve(ctx context.Context,question,pathPrefix string,limit int)([]Result,error){
	...
	for _,c:=range chunks{s,err:=embedding.Cosine(qv[0],c.Vector);if err!=nil{continue};vectors=append(vectors,scored{c,s})}
	...
	all:=make([]*combined,0,len(byID));for _,c:=range byID{all=append(all,c)};sort.SliceStable(all,func(i,j int)bool{if all[i].score==all[j].score{return all[i].vector>all[j].vector};return all[i].score>all[j].score});if len(all)>limit{all=all[:limit]}
