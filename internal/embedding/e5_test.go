...
func TestE5MultilingualQueries(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.q, func(t *testing.T) {
			q, err := e.EmbedQuery(ctx, []string{tc.q})
			if err != nil {
				t.Fatal(err)
			}
			best := -1
			...
			if best != tc.want {
				t.Fatalf("best passage %d, want %d", best, tc.want)
			}
			var norm float64
			for _, v := range q[0] {
...
func TestTokenWindowsRetainTailAndPrefix(t *testing.T) {
	...
	for _, w := range windows {
		if len(w) > 512 || w[0] != 0 || w[1] != 123 || w[2] != 456 || w[len(w)-1] != 2 {
			t.Fatalf("invalid window: %v", w)
		}
		body = append(body, w[3:len(w)-1]...)
	}
	if len(body) != 1500 {
		t.Fatalf("lost tokens: %d", len(body))
	}
	for i, v := range body {
		if v != int64(i+1000) {
...
func TestE5LongTextIncludesTail(t *testing.T) {
	...
	similarity, err := Cosine(a[0], a[1])
	if err != nil {
		t.Fatal(err)
	}
	if similarity > .999 {
		t.Fatalf("long tail was ignored: similarity %g", similarity)
	}
}
...
func TestE5EmptyBatchAndCancellation(t *testing.T) {
	...
	cancel()
	if _, err = e.EmbedQuery(ctx, []string{"hello"}); !errors.Is(err, context.Canceled) {
		t.Fatalf("got %v", err)
	}
}
...
func TestE5ConcurrentCalls(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		go func() {
			defer wg.Done()
			if _, err := (E5{}).EmbedQuery(context.Background(), []string{"Find the train timetable"}); err != nil {
				t.Error(err)
			}
		}()
	}
...
func TestAssetValidationRejectsMissingAndCorruptFiles(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, "model.onnx"), []byte("corrupt"), 0600); err != nil {
		t.Fatal(err)
	}
	if err := verifyAssets(dir); err == nil || !strings.Contains(err.Error(), "checksum") {
		t.Fatalf("got %v", err)
	}
}
...
func BenchmarkE5Query(b *testing.B) {
	e := E5{}
	ctx := context.Background()
	if _, err := e.EmbedQuery(ctx, []string{"warmup"}); err != nil {
		b.Fatal(err)
	}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if _, err := e.EmbedQuery(ctx, []string{"Quanti giorni di ferie spettano ai dipendenti?"}); err != nil {
			b.Fatal(err)
		}
	}
}
...
func TestE5MatchesIndependentReference(t *testing.T) {
	...
	for _, tc := range cases {
		var got [][]float32
		if tc.Kind == "query" {
			got, err = (E5{}).EmbedQuery(context.Background(), []string{tc.Text})
		} else {
			got, err = (E5{}).Embed(context.Background(), []string{tc.Text})
		}
		if err != nil {
			t.Fatal(err)
		}
