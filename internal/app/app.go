...
import (
	...
	"github.com/gibbok/chiedi/internal/mcp"
...
func Run(ctx context.Context,args []string,stdin io.Reader,stdout,stderr io.Writer)int{
	global:=flag.NewFlagSet("chiedi",flag.ContinueOnError);global.SetOutput(stderr);defaultDB:=os.Getenv("CHIEDI_DB");if defaultDB==""{defaultDB="chiedi.db"};dbPath:=global.String("db",defaultDB,"SQLite index path")
	...
	case "add":if len(rest)!=2{return fail(errors.New("usage: chiedi add <directory>"))};if err:=s.AddRoot(ctx,rest[1]);err!=nil{return fail(err)};fmt.Fprintln(stdout,"added",rest[1]);return 0
	case "remove":if len(rest)!=2{return fail(errors.New("usage: chiedi remove <directory>"))};if err:=s.RemoveRoot(ctx,rest[1]);err!=nil{return fail(err)};fmt.Fprintln(stdout,"removed",rest[1]);return 0
...
func printJSON(w io.Writer,v any,stderr io.Writer)int{enc:=json.NewEncoder(w);enc.SetIndent("","  ");if err:=enc.Encode(v);err!=nil{fmt.Fprintln(stderr,"chiedi:",err);return 1};return 0}
func usage(w io.Writer){fmt.Fprintln(w,"usage: chiedi [--db PATH] <init|add|remove|roots|index|reconcile|search|status|doctor|mcp|version>")}
