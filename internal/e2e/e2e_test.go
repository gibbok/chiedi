...
func TestBuiltBinaryCommonUseCase(t *testing.T){
	...
	testMCP(t,binary,db)
	writeAt(t,filepath.Join(corpus,"hr.md"),"# Employee handbook\n\n## Annual leave\n\nEmployees are entitled to twenty-five business days of paid annual leave each year.\n",time.Now().Add(2*time.Second));modified:=run("reconcile");if !strings.Contains(modified,`"embedding_jobs": 1`){t.Fatalf("modify did not embed one chunk:\n%s",modified)};if got:=run("search","vacation allowance");!strings.Contains(got,"twenty-five"){t.Fatalf("updated content missing:\n%s",got)}
